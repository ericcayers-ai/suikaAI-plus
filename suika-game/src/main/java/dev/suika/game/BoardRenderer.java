...
public final class BoardRenderer {
    ...
    // --- Board transform (chosen so the 10×15 container is centred with head-room) ---
    ...
    /** Screen/virtual x of discrete drop column {@code i} of {@code bins}. */
    ...
    public void drawBoard(ShapeRenderer s, GameState gs, GameSettings cfg, Particles particles) {
        float wallT  = vpr(PhysicsConfig.WALL_THICKNESS);
        float innerL = vpx(0);
        float innerR = vpx(PhysicsConfig.CONTAINER_WIDTH);
        float floorY = vpy(0);
        float topY   = vpy(PhysicsConfig.CONTAINER_HEIGHT);
        float wellW  = innerR - innerL;
        float wellH  = topY - floorY;

        // Well interior
        s.setColor(Theme.WELL);
        Ui.fillRoundRect(s, innerL, floorY, wellW, wellH, 10f);

        // Walls (left, right, floor) drawn as rounded bars
        s.setColor(Theme.WALL);
        ...
        // wall highlight
        s.setColor(Theme.WALL_HI);
        Ui.fillRoundRect(s, innerL - wallT, topY - 4f, wellW + 2 * wallT, 4f, 2f);

        // Dead-line
        float dlY = vpy(PhysicsConfig.DEADLINE_Y);
        ...
        s.setColor(dl.r, dl.g, dl.b, dl.a * pulse);
        s.rect(innerL, dlY - 2f, wellW, 4f);
        // little danger ticks
        s.setColor(dl.r, dl.g, dl.b, dl.a * 0.5f * pulse);
        s.rect(innerL, dlY - 6f, wellW, 2f);

        // Drop guide + ghost fruit
        if (cfg.showGuide && !gs.gameOver() && !Float.isNaN(hoverX) && hoverTier != null) {
            float gx = vpx(hoverX);
            float ghostR = vpr(hoverTier.radius);
            // dashed guide line down to the floor
            s.setColor(1f, 1f, 1f, 0.10f);
            float yy = vpy(PhysicsConfig.DROP_Y);
            while (yy > floorY) {
                s.rect(gx - 1.5f, yy, 3f, 12f);
                yy -= 24f;
            }
            // ghost preview at the drop height
            drawFruit(s, gx, vpy(PhysicsConfig.DROP_Y), ghostR, hoverTier, cfg, 0.45f);
        }

        // Fruits (glossy, depth-shaded)
        for (Fruit f : gs.fruits()) {
            drawFruit(s, vpx(f.x()), vpy(f.y()), vpr(f.radius()), f.tier(), cfg, 1f);
        }

        // Merge particles
        if (cfg.particles && particles != null) particles.render(s);
    }
    ...
    public void drawLabels(SpriteBatch b, BitmapFont font, GameState gs, GameSettings cfg) {
        ...
        for (Fruit f : gs.fruits()) {
            ...
            gl.setText(font, t);
            // dark label for light fruit, light label for dark fruit
            ...
            font.draw(b, t, vpx(f.x()) - gl.width / 2f, vpy(f.y()) + gl.height / 2f);
        }
    }
}
